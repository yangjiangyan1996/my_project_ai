...
//                password, email, Const.ROLE_DEFAULT, new Date());
    ...
    @Override
    public Account selectBySecrecyId(Long secrecyId) {
        return this.baseMapper.selectOne(new QueryWrapper<Account>()
                .eq("secrecy_id", secrecyId)
                .eq("is_deleted", 0));
    }
    ...
    public List<Account> selectFildByUserId() {
        return this.baseMapper.selectList(new QueryWrapper<Account>()
                .select("id", "username",  "avatar_url", "sex" , "province", "city", "county")
                .eq("is_deleted", 0));
    }
