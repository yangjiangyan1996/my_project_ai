...
public interface CkStockLockService extends IService<StockLock> {
    
    /**
     * 根据来源单据ID查询锁定记录
     */
    List<StockLock> findBySourceId(Long tenantId, Long sourceId);
    
    /**
     * 根据来源单据ID和锁定类型查询
     */
    List<StockLock> findBySourceIdAndLockType(Long tenantId, Long sourceId, Integer lockType);
     ...
     * 查询过期的锁定记录
     */
    List<StockLock> findExpiredLocks();
}